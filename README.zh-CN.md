...
2. Qwen 保留原生约 8 秒编码块；Audio Smith 在后台使用 16 秒滑动精修窗口。25% overlap 自动得到 4 秒重叠和 12 秒步长，界面仍只显示波形。
3. 不超过 16 秒的请求在松开快捷键后只执行一次整段最终识别；不足 8 秒的输入仅在送入模型时补尾部静音，真实时长不变。更长的请求只解码最后一个带重叠的尾部窗口；若窗口拼接置信度不足，则安全回退到一次整段解码。
